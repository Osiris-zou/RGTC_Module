...
def main():
 ...
 model=load_model(a.model,a.method,a.r,a.beta,a.checkpoint,a.pretrained,torch.device(a.device)); n=c1=c5=0
 with torch.inference_mode():
  for x,y in tqdm(dl):
   out=model(x.to(a.device)); y=y.to(a.device); n+=y.numel(); pred=out.topk(5,1).indices; c1+=(pred[:,0]==y).sum().item(); c5+=(pred==y[:,None]).any(1).sum().item()
